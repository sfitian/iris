<?php include("header.php") ?>
 <!--==========================
      Sponsors Section
    ============================-->
    <section id="supporters" class="ftco-section bg-light">

<div class="container">
    <div class="section-header">
    <div class="row justify-content-center pb-5">
          <div class="col-md-12 heading-section text-center ftco-animate">
            <h2 class="mb-4">Sponsors</h2>
          </div>
        </div>
    </div>
<style>
	.IOlogo{
		width:75%;
		height:100%;
	}
	</style>
    <div class="row no-gutters supporters-wrap clearfix">
        <!-- <div  class="col-lg-3 col-md-4 col-xs-6 mb-3"></div> -->

		<div class="col-lg-3 col-md-2 col-xs-6 mb-3 " id="margin">
            <div class="supporter-logo image1 card1 mx-auto">
                <img src="images/sponsors/IO.png" class="img-fluid IOlogo" alt="">
            </div>
        </div>
        <div class="col-lg-3 col-md-4 col-xs-6 mb-3" id="margin">
            <div class="supporter-logo card1 mx-auto image1 padddpl">
                <img src="images/sponsors/DDPL.png" class="img-fluid" alt="">
            </div>
        </div>
        <style>
             @media (min-width: 768px)  {
            .padvd{
                padding-top:25%;
                padding-bottom:25%;
            }
        }
        @media (min-width: 768px) {
            .padddpl{
                padding-top:13%;
                padding-bottom:13%;
            }
        }
            </style>
        <div class="col-lg-3 col-md-4 col-xs-6 mb-3" id="margin">
            <div class="supporter-logo card1 mx-auto image1 padvd">
                <img src="images/sponsors/VD.png" class="img-fluid" alt="">
            </div>
        </div>

        

        <div class="col-lg-3 col-md-4 col-xs-6 mb-3 "id="margin">
            <div class="supporter-logo card1 mx-auto image1">
                <img src="images/sponsors/KS.jpg" class="img-fluid" alt="">
            </div>
        </div>

        <div class="col-lg-3 col-md-4 col-xs-6 mb-3"id="margin">
            <div class=" jutify-content-center suppoter-logo card1 mx-auto image1">
                <img src="images/sponsors/AP.jpg" class="img-fluid" alt="">
            </div>
        </div>
    </div>

</div>
</section>

<?php include("footer.php") ?>




